...
def generatestructcppheadercode(codetree):
  """struct header"""
  cppheadercodetemplate = "./UserApiStruct.h.template"
  cppheadercodefile = "./UserApiStruct.h"
  cppheadercode = ""
...
def generatestructcppsourcecode(codetree):
  """struct source"""
  cppsourcecodetemplate = "./UserApiStruct.cpp.template"
  cppsourcecodefile = "./UserApiStruct.cpp"
  cppsourcecode = ""
  for (key,value) in codetree.items():
    if(value['type'] == 'struct'):
...
def generatemacrocode(codetree):
  template = "./UserApiDataType.cpp.template"
  file = "./UserApiDataType.cpp"
  cppsourcecode = ""
  for (key,value) in codetree.items():
    if(value['type'] == 'enum'):
...
def generateMdApi(codetree):
  header_template = "./MdApi.h.template"
  source_template = "./MdApi.cpp.template"
  
  #for method in codetree['CThostFtdcMdSpi']['methods']:
    #print(method)

import os
ctpdir = '../api/6.3.19_P1_linux64'
codetree = {}
